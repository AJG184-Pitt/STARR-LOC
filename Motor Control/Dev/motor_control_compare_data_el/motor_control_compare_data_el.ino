...
void setup() {
  ...
  // move in a circle and compare the step number and azimuth angle  
  // spin around the circle and print out mag measurements
  uint16_t el_step_inc = 50;

  el_step_control(1,steps_per_revolution/4);
  int el_step = -steps_per_revolution/4;

  while(el_step <= steps_per_revolution/4){

    delay(1000);

    float el_ang = -1*get_el();
    float el_ang_real = el_step/steps_per_revolution*360;
    //if(el_ang < 0){el_ang_real -= 360;}
    float el_ang_diff = el_ang_real-el_ang;

    Serial.print(el_step);
    Serial.print("\t");
    Serial.print(el_ang);
    Serial.print("\t");
    Serial.print(el_ang_real);
    Serial.print("\t");
    Serial.print(el_ang_diff);
    Serial.print("\n");

    // move
    el_step_control(0,el_step_inc);
