...
void setup() {
  ...
  uint16_t az_step_inc = 50;
  uint16_t az_step = 0;

  while(az_step <= steps_per_revolution){

    float az_ang = get_az();
    float az_ang_real = az_step/steps_per_revolution*360;
    if(az_ang < 0){az_ang_real -= 360;}
    float az_ang_diff = az_ang_real-az_ang;

    Serial.print(az_step);
    Serial.print("\t");
    Serial.print(az_ang);
    Serial.print("\t");
    Serial.print(az_ang_real);
    Serial.print("\t");
    Serial.print(az_ang_diff);
    Serial.print("\n");

    // move
    az_step_control(0,az_step_inc);
